...
import { getCreateTransactionArgsEncoder } from '../clients/js/src/generated/types/createTransactionArgs';

async function testProperBufferedTransaction() {
  console.log('Testing PROPER buffered transaction with CreateTransactionArgs encoding...');
  ...
  // Now create the CreateTransactionArgs that should be stored in the buffer
  const createTransactionArgs = {
    accountIndex: 0,
    accountBump: smartAccountPdaBump,
    ephemeralSigners: 0,
    transactionMessage: encodedSmartAccountMessage,
    memo: null,
  };

  // Encode the CreateTransactionArgs - THIS is what goes in the buffer
  const createTransactionArgsEncoder = getCreateTransactionArgsEncoder();
  const encodedCreateTransactionArgs = createTransactionArgsEncoder.encode(createTransactionArgs);
  
  console.log(`📊 Encoded CreateTransactionArgs size: ${encodedCreateTransactionArgs.length} bytes`);
  console.log(`🔍 CreateTransactionArgs structure:`, {
    accountIndex: createTransactionArgs.accountIndex,
    accountBump: createTransactionArgs.accountBump,
    ephemeralSigners: createTransactionArgs.ephemeralSigners,
    transactionMessageSize: createTransactionArgs.transactionMessage.length,
    memo: createTransactionArgs.memo
  });

  // Calculate buffer hash of the encoded CreateTransactionArgs
  const hashBuf = await crypto.subtle.digest('SHA-256', encodedCreateTransactionArgs as unknown as ArrayBuffer);
  ...
  // Step 1: Create buffer with encoded CreateTransactionArgs
  console.log('');
  console.log('📝 Step 1: Creating buffer with encoded CreateTransactionArgs...');
  ...
  const createBufferIx = getCreateTransactionBufferInstruction({
    ...
    finalBufferSize: encodedCreateTransactionArgs.length,
    buffer: Array.from(encodedCreateTransactionArgs),
  });
  ...
  console.log('  ✅ Buffer created with CreateTransactionArgs:', createBufferSignature);
  ...
  try {
    ...
    console.log('');
    console.log('🏆 SUCCESS! Properly buffered transaction completed successfully!');
    console.log('  - Buffer created with encoded CreateTransactionArgs');
    console.log('  - Transaction created from buffer (deserialized CreateTransactionArgs)');
    ...
    console.error('❌ Properly buffered transaction failed:', error);
...
// Run the test
testProperBufferedTransaction().catch(console.error);